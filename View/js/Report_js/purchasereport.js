...
function Loadmarginreport() {
    ...
    $(".purchasereport").flexigrid({
        ...
        colModel: [{ display: 'S. No.', name: 'SN', width: 50, sortable: false, align: 'center' },
                ...
                { display: 'Principal Name', name: 'Principal_Supplier_Name', width: 300, sortable: false, align: 'left' },
		...
		{ display: 'DISCOUNT', name: 'DISCOUNT', width: 100, sortable: false, align: 'left'},
		...
		{ display: 'PACKING AMOUNT', name: 'PACKING_AMOUNT', width: 100, sortable: false, align: 'left'},
		{ display: 'INSURANCE AMOUNT', name: 'INSURANCE_AMOUNT', width: 100, sortable: false, align: 'left'},
		{ display: 'FREIGHT AMOUNT', name: 'FREIGHT_AMOUNT', width: 100, sortable: false, align: 'left'},
		{ display: 'OTHER AMOUNT', name: 'OTHER_AMOUNT', width: 100, sortable: false, align: 'left'},
                { display: 'Total', name: 'total_value', width: 100, sortable: false, align: 'left'},
...
function Search(type,value) {
	var Fromdate = $("#txtdatefrom").val();
    var Todate = $("#txtdateto").val();
	var marketseg =$("#marketsegment").val(); 
	var invoicenumber =$("#txtinvoicenumber").val(); 
	var pid =$("#principalid").val(); 
	var itemid =$("#autocomplete-ajax-codepart").val(); 
    	var path = URL + '?TYP=PURCHASEREPORTNEW'+'&todate='+Todate+'&fromdate='+Fromdate+'&marketseg='+marketseg+'&invoicenumber='+invoicenumber+'&pid='+pid+'&itemid='+itemid;
    	if (Fromdate != "" && Todate != "") {
...
function Getpdf() {
	...
	var itemid =$("#autocomplete-ajax-codepart").val(); 
    	if (Fromdate != "" && Todate != "") {
        	   window.open('purchasereport_pdf.php?todate='+Todate+'&fromdate='+Fromdate+'&marketseg='+marketseg+'&invoicenumber='+invoicenumber+'&pid='+pid+'&itemid='+itemid,'_blank');
    	}else {
        	alert("Please select date");
		} 
...
function Getexcel() {
	...
	var itemid =$("#autocomplete-ajax-codepart").val(); 
       	if (Fromdate != "" && Todate != "") {
        	   window.open('purchasereport_excel.php?todate='+Todate+'&fromdate='+Fromdate+'&marketseg='+marketseg+'&invoicenumber='+invoicenumber+'&pid='+pid+'&itemid='+itemid,'_blank');
    	}else {
        	alert("Please select date");
		}
