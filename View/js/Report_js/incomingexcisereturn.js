...
function LoadOutgoingInvoiceExcise() {
    ...
    $(".outgoing_invoice_excise_list").flexigrid({
        ...
        colModel: [{ display: 'Invoice No.', name: 'invno', width: 200, sortable: false, align: 'left' },
                ...
                { display: 'Issued By', name: 'emp', width: 200, sortable: false, align: 'left' },
                { display: 'Registration No', name: 'ecccode', width: 200, sortable: false, align: 'left' },
                { display: 'Name', name: 'princiname', width: 350, sortable: false, align: 'left' },
                { display: 'Address', name: 'princiadd', width: 150, sortable: false, align: 'left' },
...
function Getpdf() {
    ...
    var rpttype = $("#rpttype").val();
    if(Fromdate != "" && Todate != "") {
        window.open('incomingexcisereturn_pdf.php?TYP='+rpttype+'&todate='+Todate+'&fromdate='+Fromdate,'_blank');
    }
    else {
...
function Getexcel() {
  ...
  var rpttype = $("#rpttype").val();
   if (Fromdate != "" && Todate != "") {
      window.open('incomingexcisereturn_excel.php?TYP='+rpttype+'&todate='+Todate+'&fromdate='+Fromdate,'_blank');
    }else {
       alert("Please select date"); return false;
    }    
}