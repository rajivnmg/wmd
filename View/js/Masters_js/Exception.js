...
var Exception = {
	...
	ExceptionClasses: ["required", "email", "password", "url", "phonenumber", "exist", "percentage", "twochar"],
    ...
    ExceptionMessagess: {
		...
		twochar: "Value must be two characters long only.",
        exist: "Already Exist"
    },
                            ...
                            if (classnames.indexOf(val) != -1) {
                                switch (val) {
									...
									case "percentage":
                                        ...
                                        break;
									case "twochar":
                                        msgHtml = Exception.twochar(myElement, Exception.ExceptionMessagess.twochar);
                                        Exception.SetMessage(msgHtml, myElement);
                                        //traversingCount++;
                                        return Exception.validStatus;
                                        break;
	...
	/* validate string length of two characters */
	twochar: function (element, message) {
		var value = $(element).val();
		if(value.length != 2)
		{
			html = Exception.getMessageHtml(message, 'error', element);
