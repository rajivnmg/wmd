	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			inPlaceSort: function() {
			    ...
			    // then sorting based on that?  yeah.
			...
			InPlaceSearch: function(){
				...
				// execute the search
				//	jQuery('input[name=q]', g.sDiv).keyup(function() {					
					    ...
					    if ($('input[name=q]', g.sDiv).val().length > 2) {				
					        ...
					        $(rows).find(' div:Contains('+searchstring+')').parent().parent().show();
					       	jQuery(this.bDiv).find('.norecords').remove();			
					    }					
					    ...
					    if ($(this.bDiv).find("tr:visible").length == 0) {		
					        ...
					        jQuery(this.bDiv).append('<div class="norecords">No records were found</div>');					
