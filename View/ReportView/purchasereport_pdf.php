    ...
    { 
        ...
        { //width
			if($eachResult['total_value'] != "Total : "){
				...
				$this->Cell(40,6,$item_Desc,0);
				$this->Cell(15,6,$eachResult["qty"],0);
				$this->Cell(18,6,number_format((float)$eachResult["unitrate"], 2, '.', ''),0);
				$this->Cell(18,6,number_format((float)$eachResult["CGST_AMOUNT"], 2, '.', ''),0);
				$this->Cell(18,6,number_format((float)$eachResult["SGST_AMOUNT"], 2, '.', ''),0);
				$this->Cell(18,6,number_format((float)$eachResult["IGST_AMOUNT"], 2, '.', ''),0);
				$this->Cell(18,6,number_format((float)$eachResult["basic_value"], 2, '.', ''),0);
				...
				$this->Cell(18,6,'',0);
				$this->Cell(18,6,'',0);
				$this->Cell(18,6,$eachResult["total_value"],'',0);
				$this->Cell(25,6,number_format((float)$eachResult["bill_value"], 2, '.', ''),0);
				$this->Ln();
...
$header=array('SNo','Invoice No','Invoice Date','Principal Name','CodePart','CodePart Desc','Qty','Basic Price','CGST','SGST','IGST','Basic Val.','Invoice Value');
//Data loading
//*** Load MySQL Data ***//
...
$pdf->Cell(55);
$pdf->Write(5, 'Purchase Report : From  '.date("d-m-Y", strtotime($_REQUEST['todate'])).' To '.date("d-m-Y", strtotime($_REQUEST['fromdate'])).'');
$pdf->Ln();
$pdf->Cell(25);
$pdf->Write(5, $CompanyInfo["Name"].' ,'.$CompanyInfo["Address"]);
$pdf->Ln();
$pdf->SetFillColor(255,255,255);
$w=array(10,23,23,40,24,40,15,18,18,18,18,18,25);
