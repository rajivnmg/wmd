        ...
        echo'<table class="table table-striped table-bordered table-hover">
					...
					if(sizeof($Print) > 0){					
						...
						$findme   = 'E';
							...
							echo'<td>'.$row['oinv_date'].'</td><td>'.$row['Item_Code_Partno'].'</td><td>'.$row['Item_Desc'].'</td><td>'.$row['ordered_qty'].'</td><td>'.$row['issued_qty'].'</td><td>'.($row['ordered_qty'] - $row['issued_qty']).'</td><td>'.$row['bill_value'].'</td></tr>';
