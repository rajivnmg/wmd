	...
	if(mysql_num_rows($result) > 0)
	{
            while($row = mysql_fetch_row($result))
            {
                ...
                $data[$counter] = array('SN'=>++$i,'oinvoice_No'=>$row[1],'oinv_date'=>$row[2],
                    ...
                    'issued_qty'=>$row[7],'UNITNAME'=>$row[8],'po_price'=>$po_price,'salesExecutive'=>$row[12],'totalprice'=>($row[7] * $po_price),'marketsegment'=>$marketsegment);
                $counter++;
...
WHERE transaction.transactionId IN (SELECT MAX(transaction.transactionId) FROM transaction 
LEFT JOIN transaction_detail ON transaction.transactionId = transaction_detail.transactionId
WHERE (`transactionType`='IE' OR `transactionType`='OE' OR  `transactionType`='Stock') AND transaction_detail.code_partNo='".$itemid."' AND `transactionDate` BETWEEN '$todate' AND '$fromdate'  GROUP BY refId)";
