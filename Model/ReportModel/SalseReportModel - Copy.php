...
class SalseReportModel
{
	...
	public static function getDailySalesReport($todate,$fromdate,$tag,$value){		
		$Query="SELECT oe.oinvoice_No,oe.oinvoice_exciseID as id,oe.oinv_date,oe.bill_value
		FROM outgoinginvoice_excise AS oe WHERE oe.oinv_date BETWEEN '$todate' AND '$fromdate'
		UNION SELECT one.oinvoice_No,one.oinvoice_nexciseID as id,one.oinv_date,one.bill_value FROM outgoinginvoice_nonexcise AS one 
		WHERE one.oinv_date BETWEEN '$todate' AND '$fromdate' "; 	
		$result = DBConnection::SelectQuery($Query); 	
		$data = array();
		$counter = 0;
		$i=0;
        ...
        if(mysql_num_rows($result) > 0){		
			 while($row = mysql_fetch_array($result,MYSQL_ASSOC)){		 
				$data[$counter] = array('SN'=>++$i,'oinvoice_No'=>$row['oinvoice_No'],'oinv_date'=>$row['oinv_date'],'bill_value'=>$row['bill_value']);					
				$total_val = $total_val + $row['bill_value'];
				$counter++; 
			}			
			$data[$counter] = array('SN'=>'','oinvoice_No'=>'','oinv_date'=>'Total : ','bill_value'=>$total_val);
		}   
	...
	public static function GetPurchaseReport($todate,$fromdate,$tag,$value){		
		...
		$Query1="(SELECT ie.principal_inv_no as invoice,ie.principal_inv_date as invoiceDate,ie.total_bill_val as totalbill,ied.p_qty as qty,ied.ed_perUnit as unitrate,psm.Principal_Supplier_Name,im.Item_Code_Partno,im.Item_Desc,ie.msid FROM incominginvoice_excise AS ie 
