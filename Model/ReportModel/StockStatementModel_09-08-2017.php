...
where oie.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by oie.oinvoice_No ASC";
	...
	if(mysql_num_rows($result) > 0)
	{
            while($row = mysql_fetch_assoc($result))
            {
				...
				{
					$data[$counter] = array('SN'=>++$i,'oinvoice_No'=>$row['oinvoice_No'], 'oinv_date'=>$row['oinv_date'],'BuyerCode'=>$row['BuyerCode'], 'BuyerName'=>$row['BuyerName'],'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'], 'issued_qty'=>$row['issued_qty'], 'UNITNAME'=>$row['UNITNAME'], 'po_price'=>$row['oinv_price'],'salesExecutive'=>$row['executiveId'],'totalprice'=>$row['total'],'marketsegment'=>$marketsegment,'PRINCIPAL_NAME'=>$row['Principal_Supplier_Name'], 'DISCOUNT'=>number_format($row['DISCOUNT'], 2, '.', ''), 'CGST_AMOUNT'=>number_format($row['cgst_amt'], 2, '.', ''), 'SGST_AMOUNT'=>number_format($row['sgst_amt'], 2, '.', ''), 'IGST_AMOUNT'=>number_format($row['igst_amt'], 2, '.', ''), 'TAXABLE_AMOUNT'=>number_format($row['taxable_amt'], 2, '.', ''), 'INVOICE_VALUE'=>'---');
				}
				else
				{
					$data[$counter] = array('SN'=>++$i,'oinvoice_No'=>$row['oinvoice_No'], 'oinv_date'=>$row['oinv_date'],'BuyerCode'=>$row['BuyerCode'], 'BuyerName'=>$row['BuyerName'],'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'], 'issued_qty'=>$row['issued_qty'], 'UNITNAME'=>$row['UNITNAME'], 'po_price'=>$row['oinv_price'],'salesExecutive'=>$row['executiveId'],'totalprice'=>$row['total'],'marketsegment'=>$marketsegment,'PRINCIPAL_NAME'=>$row['Principal_Supplier_Name'], 'DISCOUNT'=>number_format($row['DISCOUNT'], 2, '.', ''), 'CGST_AMOUNT'=>number_format($row['cgst_amt'], 2, '.', ''), 'SGST_AMOUNT'=>number_format($row['sgst_amt'], 2, '.', ''), 'IGST_AMOUNT'=>number_format($row['igst_amt'], 2, '.', ''), 'TAXABLE_AMOUNT'=>number_format($row['taxable_amt'], 2, '.', ''), 'INVOICE_VALUE'=>number_format($row['taxable_amt'], 2, '.', ''));
            ...
            $data[$counter] = array('SN'=>'','oinvoice_No'=>'','oinv_date'=>'',
                    ...
                    'issued_qty'=>'','UNITNAME'=>'','po_price'=>'','salesExecutive'=>'','totalprice'=>$total,'marketsegment'=>'','PRINCIPAL_NAME'=>'', 'DISCOUNT'=>'', 'CGST_AMOUNT'=>'', 'SGST_AMOUNT'=>'', 'IGST_AMOUNT'=>'Gross Total', 'TAXABLE_AMOUNT'=>'', 'INVOICE_VALUE'=>number_format($total_invoice, 2, '.', ''));
            $counter++;
