...
$logger = Logger::getLogger('OINE_controller');
...
switch($Type){
    case QueryModel::INSERT:
		...
		$bill_value = Outgoing_Invoice_NonExcise_Model::ValidateOutgoingInvoiceNonExciseDataCalculation($Data);
		if($bill_value == 0){
        ...
        if($Print == 0)
        {
            ...
            {
                Outgoing_Invoice_NonExcise_Model_Details::InsertOutgoingInvoiceNonExciseDetails($Print,$obj->_items[$i]->bpod_Id,$obj->_items[$i]->bpod_Id,$obj->_items[$i]->_item_id,$obj->_items[$i]->ordered_qty,$obj->_items[$i]->issued_qty,$obj->_items[$i]->bpod_Id);
                ...
                if($tranId>0)
                {
					...
					$trxndid = TransactionDetails_Model::InsertTransactionDetails($tranId,$obj->_items[$i]->_item_id,"ON",$obj->_items[$i]->issued_qty,"d",$Balanceqty);
    ...
    case QueryModel::SELECT:
        $oinvoice_exciseID = $_REQUEST['OutgoingInvoiceNonExciseNum'];
...
function Search($year,$oinv,$Todate,$Fromdate,$Principalid,$Buyerid){
    ...
    $qtype = isset($_POST['qtype']) ? $_POST['qtype'] : false;
    $count = 0;
    $rows = Outgoing_Invoice_NonExcise_Model::SearchOutgoingInvoiceNonExcise($year,$oinv,$Todate,$Fromdate,$Principalid,$Buyerid,$start,$rp,$count);
    header("Content-type: application/json");
    $jsonData = array('page'=>$page,'total'=>0,'rows'=>array());
    $i = 0;
    foreach($rows AS $row){
