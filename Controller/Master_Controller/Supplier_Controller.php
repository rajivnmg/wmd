...
$logger = Logger::getLogger('Supplier_controller');
$Type = $_REQUEST['TYP'];
...
switch($Type){
    case QueryModel::INSERT:
        $Data = $_REQUEST['SUPPLIERDATA'];
        $Data = str_replace('\\','', $Data);
        $obj = json_decode($Data);
        $logger->debug($Data);	// to create debug type log file
        //echo($obj->{'_principal_supplier_name'});
        //exit();
    ...
    case QueryModel::SELECT:
        $_principal_supplier_id = $_REQUEST['SUPPLIERID'];
        $result = Principal_Supplier_Master_Model::Load_Principal_Supplier($_principal_supplier_id,"S",1,100);
    ...
    case QueryModel::UPDATE:
        ...
        $obj = json_decode($Data);
        $logger->debug($Data);	// to create debug type log file
		/* BOF to add to add GST details for Supplier by Ayush Giri on 21-06-2017 */
		...
		/* EOF to add to add GST details for Supplier by Ayush Giri on 21-06-2017 */
    ...
    case "ADD_CONTECTPERSON":
        $Result = Pricipal_Supplier_Contact_Info::Insert_Principal_Supplier_ContactInfo($_REQUEST['PRINCIPAL_SUPPLIER_ID'].trim(),$_REQUEST['EMP_TITLE'], $_REQUEST['EMP_FNAME'], $_REQUEST['EMP_LNAME'], $_REQUEST['EMP_DEPT'], $_REQUEST['EMP_PHONE'],$_REQUEST['EMP_EMAIL']);
...
function PageLoad($_principal_supplier_id)
{
    ...
    $rows = Principal_Supplier_Master_Model::Load_Principal_Supplier($_principal_supplier_id,"S",$start,$rp);
    header("Content-type: application/json");
    $jsonData = array('page'=>$page,'total'=>0,'rows'=>array());
    $i = 0;
    foreach($rows AS $row){
        ...
        $entry = array('id' => $i,
            'cell'=>array(
                'SupplierID'       => $row->_principal_supplier_id,
                'SupplierNAME'     => $row->_principal_supplier_name,
    ...
    $jsonData['total'] = ParamModel::CountRecord("principal_supplier_master","S");
    echo json_encode($jsonData);
}
