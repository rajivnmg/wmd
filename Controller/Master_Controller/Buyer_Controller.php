...
switch($Type){
    case QueryModel::INSERT:
		...
		if($Print > 0)
        {
			...
			{
				$buyer_gst_result = Buyer_GST_Info::Insert_Buyer_GSTInfo($Print,$obj->_GSTList[$i]->_gst_state_id, $obj->_GSTList[$i]->_gst_reg_status, $obj->_GSTList[$i]->_gst_mig_status, $obj->_GSTList[$i]->_gst_no, $obj->_GSTList[$i]->_gst_reg_date, $obj->_GSTList[$i]->_arn_no, $obj->_GSTList[$i]->_perm_gst);
				$i++;
				$buyer_gst_result .= $buyer_gst_result.';;;';
			} 
			/* EOF to add to add GST details for Buyer by Ayush Giri on 13-06-2017 */
        }
        //echo json_encode($Print);
		echo json_encode($buyer_gst_result);
        return;
        break;
